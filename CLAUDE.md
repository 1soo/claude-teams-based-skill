...
| 개발-BE | `developer` (역할: BE) | `implementation` skill 사용(`references/spring-boot` 또는, SSR에서 Next가 지정된 경우 Next가 서버 역할 수행하므로 `references/next` 참고). DB 접근 방식(JPA/MyBatis 등)은 임의로 정하지 않고 설계자 결정(`docs/02_plan/database`)을 따른다 |
...
도메인 용어(엔티티명·상태값·약어 등)를 프로젝트 전체에서 통일해 표현하기 위해 `docs/00_context/glossary.md` 하나를 둔다. 컨텍스트 정리(compact/clear)로 이전 대화 기억이 사라져도 용어 일관성을 디스크에서 바로 복원하기 위한 문서다.
...
10. **완료 보고**: 모든 도메인 개발 완료 시 `dev-lead`가 **Main(나)에게 결과를 보고**한다.
...
5. **이력 생성 (`maintainer`)**: `maintenance` skill(`references/history-report`)로 `docs/06_maintenance/{yyyyMMdd-HHmmss}/{domain}/report.md`에 이력을 저장하고 `docs/06_maintenance/CLAUDE.md` 인덱스를 갱신한다. (`{yyyyMMdd-HHmmss}`는 `dev-lead`가 전달한 타임스탬프를 그대로 사용, 임의 생성·추측 금지)
