...
이 저장소는 요구사항 분석 → 설계 → 개발 → 테스트 전 과정을 **Claude Code의 Agent Teams**로 수행하기 위한 설정·에이전트·스킬 모음이다. 특정 서비스나 기술 스택에 종속되지 않으며, 신규 프로젝트에 그대로 적용할 수 있다. Main(사용자와 대화하는 세션)이 모든 에이전트를 소집하고, 소집 이후의 실제 협업은 에이전트끼리 `SendMessage`와 공유 task list로 직접 수행한다. 상세 규약은 [`CLAUDE.md`](./CLAUDE.md) 참고.
...
| 유지보수 | `maintainer` | 개발 완료된 시스템에 대한 추가 요구사항을 도메인별로 분석하고, 유사 유지보수 이력을 조회해 `designer`에게 전달. 유지보수 개발 완료 후 이력(`docs/06_maintenance/`)을 기록하고 직접 커밋·push. |
...
| `/sync-skill` | 이 저장소의 Agent Teams 스킬 패키지(`.claude/agents`, `.claude/commands`, `.claude/settings.json`, `.claude/skills`, `.mcp.json`, `CLAUDE.md`)를 원본 GitHub 저장소 최신 버전과 동기화. Agent를 소집하지 않고 Main이 직접 수행하며, 사용자가 직접 실행하는 명령이다. |

`/team-development`·`/resume-task`는 Main이 개입하는 경우를 다음 두 가지로 제한한다: ① 최초 킥오프(또는 재개 시 현재 단계 담당자에게 상황 전달), ② 에이전트 간 소통으로도 결론이 나지 않는 모호하거나 결정이 필요한 사안의 에스컬레이션. `/sync-skill`은 Agent Teams 워크플로우와 무관하게 Main이 단독으로 수행한다.

## 4. 산출물 구조

이 프로세스로 서비스를 구축하면 아래 구조로 산출물이 생성된다(구조 자체는 고정, 내부 내용은 프로젝트마다 다름).

```
docs/     # 분석·설계·개발계획·테스트 산출물 (01_analyze ~ 04_test), 유지보수 이력(06_maintenance)
source/
├── frontend/   # UI/FE 개발 결과물
├── backend/    # BE 개발 결과물
└── db/         # DB 스키마·초기 데이터·로컬 컨테이너 구성
```

각 디렉토리에는 그 위치의 파일·하위 구조를 설명하는 `CLAUDE.md`가 함께 생성된다.
